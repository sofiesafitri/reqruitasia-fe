<template>
    <div id="myo__experiences" class="card">
        ...
        <Modal
            ...
            name="myo__experience-modal"
            :footer="true"
        >
            <template v-slot:body>
                <div class="form__group">
                    <label class="form__label" for="company">Perusahaan</label>
                    <input
                        type="text"
                        class="form__control"
                        name="company"
                    ...
                    />
                    <span
                        v-if="errors.company || $v.form.company.$invalid"
                    ...
                    >
                        ...
                        <span v-if="$v.form.company.$error"
                            >Required field!</span
                        >
                    </span>
                </div>
                <div class="form__group">
                    <label class="form__label" for="position">Jabatan</label>
                    <input
                        type="text"
                        class="form__control"
                        name="position"
                ...
                <div class="form__group">
                    <label class="form__label" for="still_working"
                        >Masih Bekerja Disini</label
                    >
                    ...
                    <span
                        v-if="
                            errors.still_working ||
                            $v.form.still_working.$invalid
                        "
                        class="invalid-feedback"
                        role="alert"
                    >
                        <span v-if="errors.still_working">{{
                            errors.still_working[0]
                        }}</span>
                        <span v-if="$v.form.still_working.$error"
                            >Required field!</span
                        >
                    </span>
                </div>
                <div v-if="!form.still_working" class="clearfix mxn2 mb-3">
                    <div class="col col-6 px2">
                        <div class="form__group">
            ...
            <template v-slot:footer>
                <div class="flex justify-end">
                    <ModalCloseButton target="myo__experience-modal" />
                    <button
                        type="button"
                        @click="save('myo__experience-modal')"
...
export default {
        ...
        form: {
            ...
            end_month: {
                required: requiredIf(function () {
                    return !this.form.still_working;
                }),
            },
            end_year: {
                required: requiredIf(function () {
    ...
    methods: {
        ...
        fillModalForm(exp) {
            ...
            this.form.still_working = exp.still_working;
            ...
            this.$bus.$emit("show-modal", "myo__experience-modal");
        },
        blankModalForm() {
            this.form_mode = "add";
            this.form.id = 0;
        ...
        save(modal) {
            ...
            this.form.end_month = this.form.still_working
                ? null
                : this.form.end_month;
            this.form.end_year = this.form.still_working
                ? null
                : this.form.end_year;

            if (this.form_mode == "edit") {
                this.$axios
                    .$patch(`account/experiences/${this.form.id}`, this.form)
                    .then((res) => {
                        let user = res.data;
                        this.resetMonth();
                        this.$emit("updated", user);
                        this.swalToast(res.status, res.message);
                        this.$bus.$emit("close-modal", "myo__experience-modal");
                    })
                    .catch((err) => {
                        this.axiosError(err);
                    })
                ...
                this.$axios
                    .$post(`account/experiences`, this.form)
                    .then((res) => {
                        let user = res.data;
                        this.resetMonth();
                        this.$emit("updated", user);
        ...
        deleteExp(exp, index) {
            this.$swal({
                ...
                if (result.isConfirmed) {
                    this.$axios
                        .$delete(`account/experiences/${exp.id}`)
                        .then((res) => {
                            let user = res.data;
                            this.$emit("updated", user);
                        })
